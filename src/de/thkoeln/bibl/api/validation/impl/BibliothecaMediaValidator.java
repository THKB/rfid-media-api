...
/**
 * Implements a validator for bibliotheca tags which checks against
 * a SISIS media object.
 * 
...
public class BibliothecaMediaValidator <T extends AbstractMediaNumber>
	...
	/**
	 * Initialize a new BibliothecaMediaValidator with required the
	 * information.
	 * 
	 * @param afiMap a map with AFI tag values and the associated 
	 * borrow state
	 * @param loanMap a map with status tag values and the associated
	 * loanablility
	 */
	public BibliothecaMediaValidator(Map<Integer, BorrowState> afiMap, 
									 Map<Integer, LoanAbility> loanMap) {
		this.afiMap = afiMap;
		this.loanMap = loanMap;
	}

	/**
	 * Validates the Bibliotheca tag against the SISIS media and vice versa.
	 * The validation includes signature validation, media type validation, 
	 * media parts validation, loanability status validation and borrow state 
	 * validation if the tag supports the AFI.
	 */
	@Override
	public ValidateResult<BibliothecaTag> validate(BibliothecaTag tag, SisisMedia<T> media) {
		...
		if (!data.getSignature().equals(media.getMediaNumber().getNumber()))
			re.add("media signature differs '%s' <> '%s'", 
					media.getMediaNumber(), 
					data.getSignature());
		
		// verify media type
		if (data.getType() != media.getType())
			re.add("media type differ '%d' <> '%d'", 
					media.getType(), 
					data.getType());
		
		// verify media parts
		if (data.getParts() != media.getParts())
			re.add("media parts differ '%d' <> '%d'", 
					media.getParts(), 
					data.getParts());
		
		// verify media loanability
		if (loanMap.get(data.getStatus()).isLoanable() != 
						media.getLoanAbility().isLoanable())
			re.add("media loanability differ '%s' <> '%s'", 
					media.getLoanAbility(), 
					loanMap.get(data.getStatus()));
		
		// skip AFI check if not supported
		if (!tag.supportsAFI()) return re;
		
		// verify media borrow state		
		if (getOrDefault(afiMap, tag.getAFI(), BorrowState.UNKNOWN).isBorrowed() != 
								media.getBorrowState().isBorrowed())
			re.add("media borrow state differ '%s' <> '%s'", 
					media.getBorrowState(),
	...
	/**
	 * Returns the value from the supplied map to which the specified key 
	 * is mapped, or a default value if the map contains no mapping for 
	 * the key.
	 * 
